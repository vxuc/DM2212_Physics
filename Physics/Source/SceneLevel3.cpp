...
void SceneLevel3::Init()
{
	...
	enemyStation->type = GameObject::GO_ENEMY_BOSS;
	enemyStation->isHit = true;
	enemyStation->health = 75;
	enemyStation->scale = Vector3(20, 20, 5);
	enemyStation->pos = Vector3(m_worldWidth * 0.5f, m_worldHeight * 0.5f, 1);
	...
	for (int i = -1; i < 2; i += 2)
	{
		...
		enemyTurret->health = 25;
		enemyTurret->scale = Vector3(5, 5, 5);
		enemyTurret->pos = enemyStation->pos + Vector3(-50 * i, -30, 0);
	}
...
void SceneLevel3::SplitAsteroid(GameObject* go)
{
...
void SceneLevel3::UpdatePowerUp(double dt)
{
...
void SceneLevel3::UpdateKeyInputs(double dt)
{
	m_force.SetZero();
	m_torque.SetZero();
...
void SceneLevel3::UpdateMouseInputs(double dt)
{
	//Mouse Section
...
void SceneLevel3::UpdateCamera(double dt)
{
	//move camera pos
...
void SceneLevel3::SpawnPowerUp(GameObject* go, float rng)
{
...
bool SceneLevel3::LevelOne(double dt)
{
	...
	if (enemiesAlive <= 0 && enemiesKilledTime > 1 && SceneBase::missionType == ELIMINATE_REMAINING_ENEMIES_LEVEL3)
	{
		SceneBase::flashingTime = 0;
		SceneBase::missionType = COMPLETED_LEVEL3;
...
bool SceneLevel3::LevelOneBonus(double dt)
{
	if (m_ship->pos.x > m_worldWidth)
	{
		SceneBase::menuType = M_WIN;
		return true;
	}
	return false;
}

void SceneLevel3::UpdateMenu()
{
	if (menuType == M_NONE)
		m_speed = 1;
	else
...
{	
	...
	if (SceneBase::missionType == COMPLETED_LEVEL3)
		LevelOneBonus(dt);
	else
		LevelOne(dt);
	...
	for (GameObject* go : m_goList)
	{
		...
		if (go->active && go->type == GameObject::GO_BULLET)
		{
			...
			for (GameObject* collidedGO : m_goList)
			{
				...
				if (collidedGO->active && collidedGO->type == GameObject::GO_ENEMY_BOSS)
				{
					if (IsCollided(go, collidedGO))
					{
						collidedGO->health--;
						bulletCount--;
						go->active = false;
						if (collidedGO->health <= 0) //Check if boss is dead
						{
							SceneBase::score += 99;
							stationCount--;
							collidedGO->active = false;
		...
		if (go->active && (go->type == GameObject::GO_ENEMY_BOSS))
		{
			go->timer += dt;

			if (go->timer > 6 && go->timer <= 6.05)
			{
	...
	{
		SceneBase::flashingTime = 0;
		SceneBase::missionType = ELIMINATE_REMAINING_ENEMIES_LEVEL3;
		stationCount = 1;
	}
}
...
void SceneLevel3::RenderGO(GameObject *go)
{
	...
	case GameObject::GO_MANA:
		...
		modelStack.PopMatrix();
		break;
	case GameObject::GO_ENEMY_BOSS:
		//Exercise 4b: render a bullet sphere with scale 0.2
		modelStack.PushMatrix();
		modelStack.Translate(go->pos.x, go->pos.y, go->pos.z);
		modelStack.Scale(go->scale.x, go->scale.y, go->scale.z);
		RenderMesh(meshList[GEO_ENEMY_BOSS], false);
		modelStack.PopMatrix();
		break;
	}
}
...
void SceneLevel3::Render()
{
	...
	{
	case DESTROY_BOSS_TURRET_LEVEL3:
		RenderMissionOnScreen("DESTROY BOSS", "AND TURRETS");		
		break;
	case ELIMINATE_REMAINING_ENEMIES_LEVEL3:
		RenderMissionOnScreen("ELIMINATE ALL", "REMAINING ENEMIES");
		break;
	case COMPLETED_LEVEL3:

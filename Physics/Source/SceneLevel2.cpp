...
void SceneLevel2::Init()
{
	...
	enemyStation->health = 30;
	enemyStation->scale = Vector3(10, 10, 5);
	enemyStation->pos = Vector3(300, 125, 0);

	GameObject* enemyTurret = FetchGO();
	enemyTurret->type = GameObject::GO_ENEMY_TURRET;
	enemyTurret->isHit = true;
	enemyTurret->health = 20;
	enemyTurret->scale = Vector3(5, 5, 5);
	enemyTurret->pos = enemyStation->pos + Vector3(0, -25, 0);

	GameObject* enemyTurret2 = FetchGO();
	enemyTurret2->type = GameObject::GO_ENEMY_TURRET;
	enemyTurret2->health = 20;
	enemyTurret2->isHit = true;
	enemyTurret2->scale = Vector3(5, 5, 5);
	enemyTurret2->pos = enemyStation->pos + Vector3(0, 25, 0);

	GameObject* enemyStation2 = FetchGO();
	enemyStation2->type = GameObject::GO_ENEMYSPACESTATION;
	enemyStation2->isHit = true;
	enemyStation2->health = 30;
	enemyStation2->scale = Vector3(10, 10, 5);
	enemyStation2->pos = Vector3(50, 50);

	GameObject* enemyTurret3 = FetchGO();
	enemyTurret3->type = GameObject::GO_ENEMY_TURRET;
	enemyTurret3->isHit = true;
	enemyTurret3->health = 20;
	enemyTurret3->scale = Vector3(5, 5, 5);
	enemyTurret3->pos = enemyStation2->pos + Vector3(-25, 0, 0);

	GameObject* enemyTurret4 = FetchGO();
	enemyTurret4->type = GameObject::GO_ENEMY_TURRET;
	enemyTurret4->health = 20;
	enemyTurret4->isHit = true;
	enemyTurret4->scale = Vector3(5, 5, 5);
	enemyTurret4->pos = enemyStation2->pos + Vector3(25, 0, 0);
...
void SceneLevel2::SplitAsteroid(GameObject* go)
{
...
void SceneLevel2::SpawnPowerUp(GameObject* go, float rng)
{
...
void SceneLevel2::UpdatePowerUp(double dt)
{
...
void SceneLevel2::UpdateKeyInputs(double dt)
{
	m_force.SetZero();
	m_torque.SetZero();
...
void SceneLevel2::UpdateMouseInputs(double dt)
{
	//Mouse Section
...
void SceneLevel2::UpdateCamera(double dt)
{
	//move camera pos
...
bool SceneLevel2::LevelOne(double dt)
{
	...
	if (enemiesAlive <= 0 && enemiesKilledTime > 1 && SceneBase::missionType == ELIMINATE_REMAINING_ENEMIES_LEVEL2)
	{
		SceneBase::flashingTime = 0;
		SceneBase::missionType = COMPLETED_LEVEL2;
		return true;
	}
...
bool SceneLevel2::LevelOneBonus(double dt)
{
	if (m_ship->pos.x > m_worldWidth)
	{
		SceneManager::activeScene = S_LEVEL3;
		SceneBase::flashingTime = 0;
		SceneBase::missionType = DESTROY_BOSS_TURRET_LEVEL3;
		return true;
	}
	return false;
}

void SceneLevel2::UpdateMenu()
{
	if (menuType == M_NONE)
		m_speed = 1;
	else
...
{	
	...
	if (SceneBase::missionType == COMPLETED_LEVEL2)
		LevelOneBonus(dt);
	else
		LevelOne(dt);
	...
	for (GameObject* go : m_goList)
	{
		...
		if (go->active && (go->type == GameObject::GO_ENEMYSPACESTATION))
		{
			...
			if (go->timer > 15)
			{
				...
				newMissile->scale = Vector3(3, 1, 1);
				newMissile->pos = go->pos;
				go->timer = 5;
			}
		...
		if (go->active && (go->type == GameObject::GO_ALLY_SHIP))
		{
			go->vel = m_ship->vel.Normalized() * 5.f;
			go->pos += go->vel * dt * m_speed;
			go->timer += dt;
			...
			Vector3 displacement = m_enemy->pos - go->pos;
			go->dir = displacement;
		...
		if (go->active && (go->type == GameObject::GO_ENEMY_TURRET))
		{
			...
			if (IsCollided(m_ship, go))
			{
				std::cout << "TURRET COLLIDED WITH M_SHIP" << std::endl;
				m_ship->health--;
		...
		if (go->active && (go->type == GameObject::GO_SHIELD))
		{
			...
			for (GameObject* collidedGO : m_goList)
			{
				if (collidedGO->active && collidedGO->type == GameObject::GO_ENEMY || collidedGO->type == GameObject::GO_ENEMY_BULLET || 
					collidedGO->type == GameObject::GO_FIGHTER_ENEMY || collidedGO->type == GameObject::GO_MISSILE)
				{
					//continue;
					if (IsCollided(go, collidedGO))
					{
	...
	if (stationCount <= 0 && turretCount <= 0)
	{
		SceneBase::flashingTime = 0;
		SceneBase::missionType = ELIMINATE_REMAINING_ENEMIES_LEVEL2;
		stationCount = 1;
	}
}
...
void SceneLevel2::Render()
{
	...
	{
	case DESTROY_ENEMYSTATION_TURRET_LEVEL2:
		RenderMissionOnScreen("DESTROY ALL ENEMY", "STATIONS & TURRETS");		
		break;
	case ELIMINATE_REMAINING_ENEMIES_LEVEL2:
		RenderMissionOnScreen("ELIMINATE ALL", "REMAINING ENEMIES");
		break;
	case COMPLETED_LEVEL2:
